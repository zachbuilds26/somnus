...
├── site/               # standalone static explainer page — deploy anywhere
...
`site/index.html` is a standalone explainer page — what Somnus is, its record, and copy-paste MCP setup for ten coding agents. It is **fully static**: one HTML file, no build step, no JavaScript that calls anything, and the backend does not serve it. Deploy it anywhere that hosts a file. Its figures are a dated snapshot with links through to the live endpoints, which are the copy to believe if the two ever disagree.
