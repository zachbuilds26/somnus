...
describe('user-trading.userMomentumBlocked', () => {
  it('never blocks on the first sighting of a window', () => {
    resetUserMidHistory();
    const r = userMomentumBlocked(SYM, 0.2, 'BUY_YES', 1_000);
    assert.equal(r.blocked, false);
  });

  it('blocks a YES buy after the book collapses against it', () => {
    resetUserMidHistory();
    userMomentumBlocked(SYM, 0.33, 'BUY_YES', 1_000);
    const r = userMomentumBlocked(SYM, 0.2, 'BUY_YES', 2_000);
  ...
  it('lets a YES buy through when the book drifts with it', () => {
    resetUserMidHistory();
    userMomentumBlocked(SYM, 0.33, 'BUY_YES', 1_000);
    const r = userMomentumBlocked(SYM, 0.4, 'BUY_YES', 2_000);
    assert.equal(r.blocked, false);
  });

  it('blocks a NO buy after the book rallies against it', () => {
    resetUserMidHistory();
    userMomentumBlocked(SYM, 0.6, 'BUY_NO', 1_000);
    const r = userMomentumBlocked(SYM, 0.72, 'BUY_NO', 2_000);
    assert.equal(r.blocked, true);
  });
  ...
  it('ignores small moves below the breaker bar', () => {
    resetUserMidHistory();
    userMomentumBlocked(SYM, 0.33, 'BUY_YES', 1_000);
    const r = userMomentumBlocked(SYM, 0.3, 'BUY_YES', 2_000);
    assert.equal(r.blocked, false);
  });
  ...
  it('forgets readings older than the window', () => {
    ...
    const r = userMomentumBlocked(SYM, 0.2, 'BUY_YES', 1_000 + 601_000);
    assert.equal(r.blocked, false);
  });
});
